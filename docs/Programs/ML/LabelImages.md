...
- Click the button `Export Image To YOLOv5`.  Select the folder with all the images/screenshots. Then select the YAML file.
 - A folder will be created `exported-<yyyymmdd>-<other numbers>`. It will have the following folder structure:
    - images
        - train
    - labels
        - train
- You will then need to add validation images, by adding a `val` folder.
- If you want to know more details regarding the dataset format, see the [YOLOv5 wiki](https://github.com/ultralytics/yolov5/wiki/Train-Custom-Data#11-create-datasetyaml).
